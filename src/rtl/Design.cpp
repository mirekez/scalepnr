...
bool Design::build(const std::string& top_module)
{
...
int Design::build_hier(Referable<Inst>* inst, Referable<Cell>& cell, int level, std::string hier_name)
{
    inst->depth = level++;
...
bool Design::connect_hier(Referable<Inst>& inst, int level)
{
    ...
    for (auto& conn : inst.conns) {  // we use internal sub_designator inside a cell
        cnt_inputs += conn.port_ref->type == Port::PORT_IN;
        cnt_outputs += conn.port_ref->type == Port::PORT_OUT;
    ...
    inst.cnt_inputs = cnt_inputs;
    inst.cnt_outputs = cnt_outputs;
    ...
    for (auto& sub_inst : inst.insts) {
        ...
        sub_inst.cnt_inputs = cnt_inputs;
        sub_inst.cnt_outputs = cnt_outputs;
    }
...
bool Design::check_conns(Referable<Inst>& inst, int level)
{
