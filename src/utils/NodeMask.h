...
struct NodeMask
{
    static constexpr int words_count = 32;
    static constexpr int bit_count = words_count * 128;
    std::array<__uint128_t, words_count> words{};

    NodeMask(__uint128_t hi = 0, __uint128_t lo = 0)
    {
        words[0] = lo;
        words[1] = hi;
    }

    NodeMask operator&(const NodeMask& v) const
    {
        NodeMask out;
    ...
    NodeMask& operator&=(const NodeMask& v)
    {
        for (int i = 0; i < words_count; ++i) {
            words[i] &= v.words[i];
        }
    ...
    bool operator==(const NodeMask& v) const
    {
